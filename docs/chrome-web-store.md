...
| `tabs` | Tells the extension when a tab is opened, and its address. A tool opened in the popup over Canvas sometimes asks for a browser window of its own — a sign-in, a viewer — which arrives as a new tab; the extension takes the address, shows the page in a popup over the one already open, and closes the tab it would otherwise have become. Used for nothing else: no history is read, kept or sent anywhere. |
