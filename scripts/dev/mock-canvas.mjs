...
let resSeq = 0;
const apptChildJson = (s, t) => ({ id: t.id, title: apptGroup.title, description: `<p>${apptGroup.description}</p>`, location_name: apptGroup.location_name, start_at: s.start_at, end_at: s.end_at, all_day: false, context_code: `user_${t.user}`, effective_context_code: 'course_101', context_name: t.user === '7' ? 'Sam Student' : 'Another Student', type: 'event', appointment_group_id: apptGroup.id, appointment_group_url: `/api/v1/appointment_groups/${apptGroup.id}`, parent_event_id: s.id, user: { id: t.user, name: t.user === '7' ? 'Sam Student' : 'Another Student' }, workflow_state: 'active', html_url: `/calendar?event_id=${t.id}` });
