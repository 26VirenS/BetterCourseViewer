...
try {
  ...
  await page.waitForSelector(su('.pz'), { timeout: 20000 });
  await page.waitForTimeout(500);
  const readyRow = await page.evaluate(() => { const r = document.querySelector('#bcv-setup').shadowRoot; const pv = r.querySelector('.pz__pv'); const wrap = r.querySelector('#pzPv'); return { tiles: [...r.querySelectorAll('#pzReady .pz__theme')].map((b) => b.dataset.ready).join(','), thumbs: r.querySelectorAll('#pzReady .pz__thumb i').length, title: r.querySelector('.pz__readyttl').textContent, on: r.querySelectorAll('#pzReady .is-on').length, pvLeft: Math.round(pv.getBoundingClientRect().left + pv.getBoundingClientRect().width / 2 - (r.querySelector('.pz').getBoundingClientRect().left + r.querySelector('.pz').getBoundingClientRect().width / 2)), clipped: pv.getBoundingClientRect().left < wrap.getBoundingClientRect().left - 1, step: r.querySelector('.pz').dataset.step }; });
  check(readyRow.tiles === 'Default,Dusk,Ocean,Forest,Sand' && readyRow.thumbs === 20 && readyRow.title === 'Ready-made' && readyRow.on === 0 && readyRow.pvLeft === -40 && !readyRow.clipped && readyRow.step === '0', `five ready-made tiles over the swatches — Default and the four scenes — none on yet, the preview 40px left of centre on the first screen and nothing of it clipped: ${JSON.stringify(readyRow)}`);
