...
**Never a broken card** – a screen that throws, that would land nothing but an error, or that has not drawn after fifteen seconds gives way to Canvas's own page for that URL, with a note saying so, rather than a card that says something failed. **The cache paints, Canvas answers** – every page load fetches each thing it shows fresh from Canvas. What the last visit left (if under a day old) is drawn first so the page appears at once, the progress bar runs while Canvas answers, and when the fresh answer differs the screen redraws in place with the scroll kept, unless you are mid-way through something (a sheet, a quiz, typing). A page left open asks Canvas again every five minutes. If Canvas does not answer, a note says the page shows what it had last time.

**Preloading** – once the page you are on has drawn and the browser is idle, every other root screen's data is fetched into the shared cache (Dashboard, Courses, Groups, To Do, Calendar, Notifications, Inbox, Grades and the course cards, each call the one that screen makes itself), two calls at a time so your own page keeps the network. It is refreshed every two minutes while the tab is visible and when the tab comes back into view, so the next page lands from the cache and draws at once.
