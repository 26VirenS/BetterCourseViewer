...
The Chrome manifest itself comes from `scripts/chrome-manifest.py` (run by `scripts/package.sh`): it drops the Firefox/Safari keys, sets `host_permissions` to `*://*/*`, and keeps the sniffer, the content script in the source manifest that runs on every site but `*.instructure.com` (Safari and Firefox run it on the sites they are allowed; Chrome on all of them). Both Chrome builds also leave out everything that would read as a new permission at an update: the `tabs` permission (*Read your browsing history*) and the on-demand modules' never-matching content-script host (Chrome lists a content script's host among the sites the extension reads). Chrome switches an installed copy **off** at an update that brings a permission with a new warning — *The newest version of "Simpl Courses" has been disabled because it requires more permissions* — until its owner presses Re-enable, and two updates (2.8 and 2.90) did exactly that. `scripts/dev/chrome-setup-test.mjs` holds each build's permission list and host list exactly, so nothing new slips into a release unnoticed. `node scripts/dev/chrome-setup-test.mjs` loads that build in Chromium and checks the whole first run against the mock Canvas at a localhost address the manifest does not name.
