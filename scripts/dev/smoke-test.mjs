...
try {
  ...
  check(sideUp.url === 'url("data:image/jpeg;base64' && /^#[0-9a-f]{6}$/.test(sideUp.veil) && sideUp.ink === inkOver(pzDark) && sideUp.note === 'Custom · 2 photos' && sideUp.upOn, `an uploaded photo goes on the sidebar, scaled here, its rows in white over the veil: ${JSON.stringify(sideUp)}`);
  ...
  check(ocean.pic && ocean.layers === 3 && ocean.title === inkOver(pzDark) && ocean.chip === 'Change' && !ocean.every && ocean.note === '1 of 9' && ocean.mask === 'linear-gradient(to left', `Ocean goes on the Dashboard's header, blurred towards the words, the title in white: ${JSON.stringify(ocean)}`);
  ...
  await page.waitForTimeout(300);
  await waitText('.bcv-stat__value', /^\d+$/);
  const picCard = await page.$eval('.bcv-stat[data-stat="today"]', (e) => ({ pic: e.classList.contains('bcv-stat--pic'), layers: e.querySelectorAll('.bcv-stat__pic').length, val: e.style.getPropertyValue('--bcv-pic').slice(0, 15), veil: e.style.getPropertyValue('--bcv-veil'), label: getComputedStyle(e.querySelector('.bcv-label')).color, mask: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).maskImage.slice(0, 15), withPics: document.querySelectorAll('.bcv-stat--pic').length, preblur: e.classList.contains('has-preblur'), blurFilter: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).filter, blurImg: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).backgroundImage.slice(0, 15), wash: getComputedStyle(e.querySelector('.bcv-stat__pic--veil')).backgroundColor }));
  check(picCard.pic && picCard.layers === 3 && picCard.val === 'url("data:image' && /^#[0-9a-f]{6}$/.test(picCard.veil) && picCard.label === inkOver(themedNow.dark) && picCard.mask === 'radial-gradient' && picCard.withPics === 1 && picCard.preblur && picCard.blurFilter === 'none' && picCard.blurImg === 'url("data:image' && /^rgba\((255, 255, 255|0, 0, 0), /.test(picCard.wash), `the Due today counter carries Dusk in three layers under a veil in the accent and the mode's wash, its blur a small picture scaled up (no filter), its words white; the other five are plain: ${JSON.stringify(picCard)}`);
  ...
  await page.waitForTimeout(500);
  const readyOn = await page.evaluate(() => { const r = document.querySelector('#bcv-setup').shadowRoot; const root = r.querySelector('.pz'); const pics = (sel) => [...r.querySelectorAll(sel)].map((e) => (e.querySelector('.pz__pic--sharp')?.style.getPropertyValue('--pic') || '').slice(0, 15)); return { on: r.querySelector('#pzReady .is-on')?.dataset.ready, colour: r.querySelector('.pz__sw.is-on').dataset.theme, A: root.style.getPropertyValue('--A'), side: pics('.pz__side').join(''), cards: pics('.pz__card'), heads: r.querySelectorAll('.pz__hcard.has-pic').length, note: r.querySelector('#pzNote').textContent, pvMain: root.style.getPropertyValue('--pv-main'), pvCard: getComputedStyle(r.querySelector('.pz__card:not(.has-pic)') || r.querySelector('.pz__list')).backgroundColor }; });
