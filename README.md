...
**Guided setup** – its own page, opened once after install (Safari and Chrome) and any time from the toolbar button or Settings: a glass card over drifting colour, one question per step. Your Canvas address (that host alone gets permission, and it must answer like Canvas before you continue); your courses, read from your enrolments with a skeleton bound to the real request, where unchecked courses stay hidden everywhere (they become your Canvas favourites, the one list every screen follows); grades (daily local snapshots on or off, a GPA goal, a target letter per course, and no question about your past record, which stays optional); the smart panel, optional, with the steps to get a Claude or ChatGPT key and a check before the key is saved (Gemini is shown as coming soon); then *Open Canvas*, which opens your site and starts the tour on arrival. Skipping never starts the tour. On iOS the same steps run inside the app.
