...
(function () {
  ...
  async function render(ctx) {
    ...
    function stat(lbl, value, note, icon, color, onOpen) {
      ...
      return U.enter(h('button', { type: 'button', class: `bcv-card bcv-stat ${pic ? 'bcv-stat--pic' : ''} ${pic?.ink ? 'has-ink' : ''}`, dataset: slot ? { stat: slot } : {}, style: pic ? { ...(pic.ink ? { '--bcv-pic-ink': BCV.theme.picCss(pic.ink), '--bcv-pic-ink-blur': BCV.theme.picCss(pic.inkBlur) } : { '--bcv-pic': BCV.theme.picCss(pic.sharp) }), '--bcv-veil': BCV.theme.veilBase(app.state?.settings?.appearance?.theme?.accent || '', app.state?.themeImages?.tones?.[slot], 0.4) } : null, onclick: (e) => onOpen(e.currentTarget) }, [
