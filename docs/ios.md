...
- **The first launch opens the guided setup** (`/?bcv=setup` on the Canvas site, after Canvas's own sign-in): the courses to favourite, the GPA goal, tracking and a target per course, the smart panel key, then a tour of the phone layout. It can be run again from the account sheet under the avatar on Today. The settings page's own "Guided setup" section (which asks for the site) is hidden in the app, like "Canvas sites".
