...
(async function () {
  ...
  // Grades page can say where the numbers came from. Tracking comes on with it. A history export
  ...
  $('recordCsvFile').addEventListener('change', async (e) => {
    ...
    paintGrades();
    flash(`${gpa2(rec.priorGpa)} across ${rec.priorCourses} ${rec.priorCourses === 1 ? 'course' : 'courses'}${rec.record.skipped ? `, ${rec.record.skipped} skipped` : ''}`);
  });
