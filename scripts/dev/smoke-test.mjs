...
try {
  ...
  check(/^F26-MATH 021 20 MATH-021-20 92\.4% A− Target − A− · 90% \+ Open the course’s Grades page by group/i.test(detail) && /how the grade is weighted 100% of final grade/i.test(detail) && /Midterms 57% · nothing graded/.test(detail) && (await page.$$('.bcv-gpa-detail__arow')).length === 17 && (await page.$$('.bcv-gpa-detail .bcv-wbar__seg--ungraded')).length === 2, `details sheet: ${detail.slice(0, 200)}`);
  ...
  const wfFields = await page.$$eval('.bcv-gpa-detail .bcv-whatif__input', (els) => ({ n: els.length, h: Math.round(els[0].getBoundingClientRect().height), w: Math.round(els[0].getBoundingClientRect().width), bg: getComputedStyle(els[0]).backgroundColor, hyp: els.filter((e) => e.classList.contains('is-hyp')).length, first: els[0].value, bannerH: Math.round(document.querySelector('.bcv-gpa-detail .bcv-banner--sm')?.getBoundingClientRect().height || 0) }));
  check(wfFields.n === 17 && wfFields.h >= 34 && wfFields.w >= 68 && wfFields.bg !== 'rgba(0, 0, 0, 0)' && !/^rgb\(255, 255, 255\)$/.test(wfFields.bg) && wfFields.hyp === 0 && wfFields.bannerH > 0 && wfFields.bannerH < 120 && (await texts('.bcv-gpa-detail .bcv-whatif-btn'))[0] === 'Exit what-if mode' && (await page.$eval('.bcv-gpa-detail__pct', (e) => e.classList.contains('is-hyp'))), `Try what-if scores: a bigger tinted field per assignment with the real score in it, the banner up: ${JSON.stringify(wfFields)}`);
  ...
  // the page after this update (setup/updated.html): the Chrome build opens it once on its first run after an update from before 2.98.15 — "Simpl has updated. Please accept the permissions." with an arrow at the puzzle piece and the line in red — it reads whether the build's site access is still withheld (Chrome withholds new site access at an update until the puzzle piece's prompt is accepted) and offers the press that asks
  const updDue = await sw.evaluate(() => [self.BCV.background.updatedPageDue('2.98.12', ['*://*/*']), self.BCV.background.updatedPageDue('2.98.14', ['*://*/*']), self.BCV.background.updatedPageDue('2.98.15', ['*://*/*']), self.BCV.background.updatedPageDue('2.98.20', ['*://*/*']), self.BCV.background.updatedPageDue('2.98.12', ['*://*.instructure.com/*']), self.BCV.background.updatedPageDue(null, ['*://*/*'])]);
  check(JSON.stringify(updDue) === '[true,true,false,false,false,false]', `the page is due once, for the build with the run of every site alone, on the first run after an update from before 2.98.15: ${JSON.stringify(updDue)}`);
  const upd = await context.newPage();
  await upd.addInitScript(() => { window.__asked = []; for (const o of [self.chrome, self.browser]) { if (!o?.permissions) continue; o.permissions.contains = () => Promise.resolve(false); o.permissions.request = (req) => { window.__asked.push(req); return Promise.resolve(true); }; } }); // (the access withheld, then granted on the press: the browser's own dialog stubbed)
  await upd.goto(`chrome-extension://${extId}/setup/updated.html`);
  await upd.waitForFunction(() => document.documentElement.dataset.state === 'ask', null, { timeout: 8000 });
  const updSeen = await upd.evaluate(() => { const arrow = document.getElementById('arrow').getBoundingClientRect(); const red = document.getElementById('never'); return { title: document.getElementById('title').textContent, ask: document.getElementById('ask').textContent, where: document.getElementById('where').textContent, whereShown: !document.getElementById('where').hidden, red: red.textContent, redColor: getComputedStyle(red).color, arrowTopRight: arrow.right > innerWidth * 0.7 && arrow.top < innerHeight * 0.35, button: document.getElementById('allow').textContent, bg: getComputedStyle(document.body).backgroundColor }; });
  check(updSeen.title === 'Simpl has updated.' && updSeen.ask === 'Please accept the permissions.' && /puzzle piece/.test(updSeen.where) && updSeen.whereShown && updSeen.red === 'Simpl never stores your data. These permissions are for functionality.' && updSeen.redColor === 'rgb(255, 69, 58)' && updSeen.arrowTopRight && updSeen.button === 'Allow Simpl to run' && updSeen.bg === 'rgb(0, 0, 0)', `the page: black, the title, the ask with an arrow up at the puzzle piece, the line in red, and Allow (${JSON.stringify(updSeen)})`);
  await upd.waitForFunction(() => document.getAnimations().every((a) => a.playState === 'finished' || a.effect?.getTiming().iterations === Infinity), null, { timeout: 5000 }); // (the fade-ins done — the arrow's nudge runs on — before the picture)
  await upd.screenshot({ path: join(out, '11e-updated-page.png') });
  await upd.click('#allow');
  await upd.waitForFunction(() => document.documentElement.dataset.state === 'ok', null, { timeout: 5000 });
  const updDone = await upd.evaluate(() => ({ asked: window.__asked, ask: document.getElementById('ask').textContent, whereShown: !document.getElementById('where').hidden, arrow: getComputedStyle(document.getElementById('arrow')).display, button: document.getElementById('allow').textContent, msg: document.getElementById('msg').textContent }));
  check(JSON.stringify(updDone.asked) === JSON.stringify([{ origins: manifest.host_permissions }]) && updDone.ask === 'All set — nothing left to accept.' && !updDone.whereShown && updDone.arrow === 'none' && updDone.button === 'Close this tab' && /reload/i.test(updDone.msg), `Allow asks for the build's sites from the press, and the page says it is done — the arrow gone, the button now closes the tab (${JSON.stringify(updDone)})`);
  await upd.close();
  await toolPop.close();
  ...
  const omniState = () => page.evaluate(() => ({ value: document.getElementById('bcv-omni').value, groups: [...document.querySelectorAll('.bcv-omni__group')].map((g) => g.dataset.group), rows: [...document.querySelectorAll('.bcv-omni__item .bcv-omni__t')].map((t) => t.textContent), cur: [...document.querySelectorAll('.bcv-omni__item')].findIndex((e) => e.classList.contains('is-cur')), more: !!document.querySelector('.bcv-omni__more'), hidden: document.getElementById('bcv-omni-panel').hidden, active: document.activeElement?.id || document.activeElement?.className || document.activeElement?.tagName }));
  const readNotes = () => fetch(`${BASE}/api/v1/planner_notes`).then((r) => r.text()).then((t) => JSON.parse(t.replace(/^while\(1\);/, '')));
  await page.click('#bcv-omni');
  await page.keyboard.type('p');
  await page.waitForSelector('#bcv-omni-panel:not([hidden]) .bcv-omni__group[data-group="Courses"] .bcv-omni__item', { timeout: 3000 });
  const one = await omniState();
  check(one.groups.join(',') === 'Courses' && one.cur === 0 && !one.more && one.rows.every((r) => /p/i.test(r)), `one letter lists the courses it is in, at once and with nothing asked of Canvas, the first chosen: ${JSON.stringify(one.rows)}`);
  await page.keyboard.type('hys');
  ...
  await eventually(async () => (await page.$('.bcv-omni__more')) === null, 12000);
  const rowActs = await page.evaluate(() => { const row = (g) => document.querySelector(`.bcv-omni__group[data-group="${g}"] .bcv-omni__item`); const labels = (g) => [...(row(g)?.querySelectorAll('.bcv-omni__act') || [])].map((b) => b.textContent.trim()); const shown = (el) => { const a = el?.querySelector('.bcv-omni__acts'); return a ? getComputedStyle(a).display : 'none'; }; return { a: labels('Assignments'), f: labels('Files'), onCur: shown(document.querySelector('.bcv-omni__item.is-cur')), onOther: shown(row('Files')) }; });
  check(rowActs.a.join(',') === 'Submit' && rowActs.f.join(',') === 'Download,Convert' && rowActs.onCur === 'flex' && rowActs.onOther === 'none', `a row carries its actions — an assignment Submit, a file Download and Convert — shown on the row chosen alone: ${JSON.stringify(rowActs)}`);
  await page.keyboard.press('ArrowRight');
  check(await page.evaluate(() => document.activeElement?.classList.contains('bcv-omni__act') && document.activeElement.textContent.trim() === 'Submit'), '→ at the end of the words moves onto the row’s first action');
  await page.keyboard.press('Escape');
  ...
  // "/" lists the commands; the letters narrow them; Tab completes the one chosen and lists what it takes
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/');
  await page.waitForSelector('.bcv-omni__group[data-group="Commands"] .bcv-omni__item', { timeout: 8000 });
  const allCmds = await omniState();
  check(allCmds.groups.join(',') === 'Commands' && allCmds.rows.length >= 20 && allCmds.rows[0] === '/submit an assignment' && allCmds.rows.includes('/dark') && allCmds.cur === 0, `"/" alone lists every command, what each takes beside it: ${allCmds.rows.length} of them, ${allCmds.rows.slice(0, 4).join(' · ')} …`);
  await page.keyboard.type('su');
  await eventually(async () => (await omniState()).rows.length < 20, 4000);
  const suRows = await page.evaluate(() => ({ rows: [...document.querySelectorAll('.bcv-omni__item .bcv-omni__t')].map((t) => t.textContent), subs: [...document.querySelectorAll('.bcv-omni__item .bcv-omni__s')].map((t) => t.textContent), hint: document.querySelector('.bcv-omni__item.is-cur .bcv-omni__hint')?.textContent }));
  check(suRows.rows[0] === '/submit an assignment' && suRows.subs[0] === 'Hand an assignment in, right here' && suRows.rows.every((r) => /^\/s/.test(r) || /submit/.test(r)) && suRows.hint === 'Tab', `the letters narrow the list — a line under each says what it does — the first chosen, with Tab to complete it: ${suRows.rows.join(' · ')}`);
  ...
  await page.keyboard.press('Enter');
  await page.waitForSelector('.bcv-hub-ov .bcv-sb__tabs', { timeout: 15000 });
  await page.click('.bcv-hub-ov .bcv-sb__tab[data-tab=text]');
  await page.fill('.bcv-hub-ov .bcv-sb__ta', 'Handed in from the search box.');
  await page.click('.bcv-hub-ov .bcv-sb__btn--primary');
  await page.waitForSelector('.bcv-hub-ov .bcv-sb__done', { timeout: 15000 });
  const hubSub = await readSub('102', '2004');
  check(hubSub.submission_type === 'online_text_entry' && hubSub.body === '<p>Handed in from the search box.</p>' && (await texts('.bcv-hub-ov .bcv-sb__donebtns .bcv-sb__btn--primary'))[0] === 'Done', `Enter opens the hand-in box on it, and a text entry reaches Canvas from there: ${JSON.stringify({ type: hubSub.submission_type, body: hubSub.body })}`);
  await shot(page, '15d-hub-submitted');
  await page.click('.bcv-hub-ov .bcv-sb__donebtns .bcv-sb__btn--primary');
  await page.waitForFunction(() => !document.querySelector('.bcv-hub-ov'), null, { timeout: 5000 });
  check(page.url() === `${BASE}/` && (await page.$('.bcv-sheet-ov')) === null, 'Done closes the box, the Dashboard still under it');
  // /download saves the course file chosen; /convert opens the converter with it; /open launches a campus tool in its own tab
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/download syll');
  await page.waitForSelector('.bcv-omni__item', { timeout: 10000 });
  const [dl] = await Promise.all([page.waitForEvent('download', { timeout: 10000 }), page.keyboard.press('Enter')]);
  check(dl.suggestedFilename() === 'syllabus.pdf' && (await page.$eval('#bcv-omni-panel', (e) => e.hidden)), `/download lists the course files by name and Enter saves the one chosen: ${dl.suggestedFilename()}`);
  await dl.delete().catch(() => {});
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/convert syll');
  await page.waitForFunction(() => document.querySelectorAll('.bcv-omni__item').length >= 2, null, { timeout: 10000 });
  const conv = await omniState();
  check(conv.rows[0] === 'A file from this device…' && conv.rows[1] === 'Course Syllabus.pdf', `/convert offers a file from this device first, then the course files: ${conv.rows.join(' · ')}`);
  await page.keyboard.press('ArrowDown');
  await page.keyboard.press('Enter');
  await page.waitForSelector('.bcv-tool[data-tool="conv"] .bcv-conv__name', { timeout: 20000 });
  check((await texts('.bcv-tool[data-tool="conv"] .bcv-conv__name'))[0] === 'Course Syllabus.pdf', 'Enter opens the converter with the course file already in it');
  await shot(page, '15e-hub-convert');
  await page.keyboard.press('Escape');
  await page.waitForFunction(() => !document.querySelector('.bcv-tool-ov'), null, { timeout: 5000 });
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/open res');
  await page.waitForSelector('.bcv-omni__item', { timeout: 10000 });
  const openRows = await omniState();
  const [hubTool] = await Promise.all([context.waitForEvent('page', { timeout: 10000 }), page.keyboard.press('Enter')]);
  ...
  check((await omniState()).rows.join(' · ') === '/grades · /groups', '"/gr" could be Grades or Groups, Grades first');
  await page.keyboard.press('Enter');
  await page.waitForFunction(() => location.pathname === '/grades' && !!document.querySelector('.bcv-screen'), null, { timeout: 10000 });
  check(page.url() === `${BASE}/grades`, 'Enter runs the one chosen: Grades opens');
  await page.goto(`${BASE}/`);
  await page.waitForSelector('#bcv-omni', { timeout: 10000 });
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/dark');
  await page.waitForSelector('.bcv-omni__item', { timeout: 8000 });
  await Promise.all([page.waitForEvent('load', { timeout: 10000 }), page.keyboard.press('Enter')]); // (a look is a fresh load of the page)
  await page.waitForSelector('#bcv-omni', { timeout: 10000 });
  check(await page.evaluate(() => document.documentElement.getAttribute('data-bcv-theme') === 'dark'), '/dark switches to the dark look, the page loaded afresh in it');
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/light');
  await page.waitForSelector('.bcv-omni__item', { timeout: 8000 });
  await Promise.all([page.waitForEvent('load', { timeout: 10000 }), page.keyboard.press('Enter')]);
  await page.waitForSelector('#bcv-omni', { timeout: 10000 });
  check(await page.evaluate(() => document.documentElement.getAttribute('data-bcv-theme') === 'light'), '/light switches back');
  await page.click('#bcv-omni');
  await page.fill('#bcv-omni', '/note read chapter 4');
  await page.waitForSelector('.bcv-omni__item', { timeout: 8000 });
  ...
  const ans1 = (await texts('.bcv-omni__group[data-group="Answer"] .bcv-omni__t'))[0];
  await page.fill('#bcv-omni', '15% of 80');
  await eventually(async () => (await texts('.bcv-omni__group[data-group="Answer"] .bcv-omni__t'))[0] === '12', 5000);
  const ans2 = (await texts('.bcv-omni__group[data-group="Answer"] .bcv-omni__t'))[0];
  check(ans1 === '14' && ans2 === '12', `a sum typed in the box is worked out on the spot: 2*(3+4) = ${ans1}, 15% of 80 = ${ans2}`);
  ...
  // the ink over a photo follows the look: the look's own dark ink on the light look, white on the dark (app.css --bcv-photo-ink)
  const inkOver = (dark) => (dark ? 'rgb(255, 255, 255)' : 'rgb(28, 28, 30)');
  const ink2Over = (dark) => (dark ? 'rgba(255, 255, 255, 0.72)' : 'rgba(28, 28, 30, 0.66)');
  ...
  // (the ink is drawn off the picture once it lands — a moment — and the preview redrawn with it)
  check(await eventually(() => page.$(pz('.pz__card[data-target="today"] .pz__pic--inked')), 6000), 'the counter is inked once its ink is drawn');
  const dusk = await page.$eval(pz('.pz__card[data-target="today"]'), (e) => ({ pic: e.classList.contains('has-pic'), layers: e.querySelectorAll('.pz__pic').length, paper: getComputedStyle(e.querySelector('.pz__pic--paper')).backgroundColor, inkColor: getComputedStyle(e.querySelector('.pz__pic--sharp')).backgroundColor, ink: e.querySelector('.pz__pic--sharp').style.getPropertyValue('--ink').slice(0, 19), veil: e.style.getPropertyValue('--veil-card'), num: getComputedStyle(e.querySelector('.pz__cn')).color, mask: getComputedStyle(e.querySelector('.pz__pic--blur')).maskImage || '', veilBg: getComputedStyle(e.querySelector('.pz__pic--veil')).backgroundImage }));
  // (the paper is the preview card's own colour — cast in the custom colour — and the ink that colour lifted a shade)
  const near = (a, b) => { const p = (c) => { const m = String(c).match(/^color\(srgb ([\d.]+) ([\d.]+) ([\d.]+)/); return m ? m.slice(1, 4).map((v) => Math.round(parseFloat(v) * 255)) : (String(c).match(/\d+/g) || []).slice(0, 3).map(Number); }; const x = p(a), y = p(b); return x.length === 3 && y.length === 3 && x.every((v, i) => Math.abs(v - y[i]) <= 2); }; // (a color-mix() computes to color(srgb …) in Chromium)
  const pvCardNow = await page.evaluate(() => document.querySelector('#bcv-setup').shadowRoot.querySelector('.pz').style.getPropertyValue('--pv-card'));
  check(dusk.pic && dusk.layers === 4 && dusk.paper === rgbOf(pvCardNow) && near(dusk.inkColor, rgbOf(THEME.inkOn(pvCardNow, pzDark))) && dusk.ink === 'url("data:image/png' && /^#[0-9a-f]{6}$/.test(dusk.veil) && dusk.num === inkOver(pzDark) && /^url\("data:image\/png/.test(dusk.mask) && dusk.mask.includes('radial-gradient(150% 150% at 100%') && dusk.veilBg === 'none' && (await page.$eval(pz('#pzNote'), (e) => e.textContent)) === 'Custom · 1 photo', `Dusk goes on the counter tone on tone — the paper the card's own colour, the ink's mask a shade lighter, the blurred mask under the fade, no veil — with the look's words over it: ${JSON.stringify({ ...dusk, mask: dusk.mask.slice(0, 40), pvCardNow })}`);
  ...
  check(await eventually(() => page.$(pz('.pz__side .pz__pic--inked')), 6000), 'the sidebar is inked once its ink is drawn');
  const sideUp = await page.evaluate(() => { const r = document.querySelector('#bcv-setup').shadowRoot; const s = r.querySelector('.pz__side'); return { url: s.querySelector('.pz__pic--sharp').style.getPropertyValue('--ink').slice(0, 26), veil: r.querySelector('.pz').style.getPropertyValue('--veil-side'), ink: getComputedStyle(s.querySelector('.pz__row span')).color, note: r.querySelector('#pzNote').textContent, upOn: r.querySelector('#pzPhotoBar .pz__choice--up').classList.contains('is-on') }; });
  check(sideUp.url === 'url("data:image/png;base64' && /^#[0-9a-f]{6}$/.test(sideUp.veil) && sideUp.ink === inkOver(pzDark) && sideUp.note === 'Custom · 2 photos' && sideUp.upOn, `an uploaded photo goes on the sidebar, scaled here, its rows in white over the veil: ${JSON.stringify(sideUp)}`);
  ...
  check(await eventually(() => page.$(pz('.pz__hcard[data-page="dashboard"] .pz__pic--inked')), 6000), 'the header is inked once its ink is drawn');
  const ocean = await page.evaluate(() => { const r = document.querySelector('#bcv-setup').shadowRoot; const c = r.querySelector('.pz__hcard[data-page="dashboard"]'); return { pic: c.classList.contains('has-pic'), layers: c.querySelectorAll('.pz__pic').length, title: getComputedStyle(c.querySelector('.pz__htitle')).color, chip: c.querySelector('.pz__hchip').textContent, every: r.querySelector('#pzEvery').disabled, note: r.querySelector('#pzNote').textContent, mask: getComputedStyle(c.querySelector('.pz__pic--blur')).maskImage || '' }; });
  check(ocean.pic && ocean.layers === 4 && ocean.title === inkOver(pzDark) && ocean.chip === 'Change' && !ocean.every && ocean.note === '1 of 9' && /^url\("data:image\/png/.test(ocean.mask) && ocean.mask.includes('linear-gradient(to left'), `Ocean goes on the Dashboard's header, blurred towards the words, the title in white: ${JSON.stringify(ocean)}`);
  ...
  const themedNow = await page.evaluate(() => ({ themed: document.documentElement.classList.contains('bcv-themed'), dark: document.documentElement.getAttribute('data-bcv-theme') === 'dark', tabIcons: [...document.querySelectorAll('.bcv-nav .bcv-nav__item')].map((e) => e.style.getPropertyValue('--bcv-tab-icon')), navColor: getComputedStyle(document.querySelectorAll('.bcv-nav .bcv-nav__item')[1]).color, h1: getComputedStyle(document.querySelector('.bcv-h1')).color, headPic: !!document.querySelector('.bcv-head.bcv-head--pic'), headLayers: document.querySelectorAll('.bcv-head__pic > i').length, headMask: (getComputedStyle(document.querySelector('.bcv-head__pic-blur') || document.body).maskImage || ''), headPicVal: (document.querySelector('.bcv-head--pic')?.style.getPropertyValue('--bcv-pic-ink') || '').slice(0, 15), headInk: document.querySelector('.bcv-head__pic')?.classList.contains('has-ink'), headPaper: getComputedStyle(document.querySelector('.bcv-head__pic') || document.body).backgroundColor, headVeil: document.querySelector('.bcv-head--pic')?.style.getPropertyValue('--bcv-veil'), sideLayers: document.querySelectorAll('.bcv-side__pic > i').length, sideUrl: (document.querySelector('.bcv-side__pic')?.style.getPropertyValue('--bcv-pic-ink') || '').slice(0, 26), sideInk: document.querySelector('.bcv-side__pic')?.classList.contains('has-ink') && document.querySelector('.bcv-side').classList.contains('bcv-side--ink'), sidePaper: getComputedStyle(document.querySelector('.bcv-side')).backgroundColor, sideVeil: document.querySelector('.bcv-side__pic')?.style.getPropertyValue('--bcv-veil'), sideBlurFilter: getComputedStyle(document.querySelector('.bcv-side__pic-blur') || document.body).filter, headBlurFilter: getComputedStyle(document.querySelector('.bcv-head__pic-blur') || document.body).filter, sideBg: getComputedStyle(document.querySelector('.bcv-side')).backgroundColor, note: JSON.parse(localStorage.getItem('bcv:early') || '{}').accent, strip: document.documentElement.classList.contains('bcv-head-pic'), barBg: getComputedStyle(document.getElementById('bcv-bar')).backgroundColor, barBlur: getComputedStyle(document.getElementById('bcv-bar')).backdropFilter, picTop: getComputedStyle(document.querySelector('.bcv-head__pic')).top, segGlass: getComputedStyle(document.querySelector('.bcv-head--pic .bcv-seg')).backdropFilter, segColor: getComputedStyle(document.querySelector('.bcv-head--pic .bcv-seg__btn:not(.is-active)')).color, footGlass: getComputedStyle(document.querySelector('.bcv-theme-btn')).backdropFilter, footColor: getComputedStyle(document.querySelector('.bcv-theme-btn')).color, activeGlass: getComputedStyle(document.querySelector('.bcv-nav__item.is-active')).backdropFilter, bg: document.documentElement.style.getPropertyValue('--bcv-bg'), cardBg: getComputedStyle(document.querySelector('.bcv-stat:not(.bcv-stat--pic)')).backgroundColor, ink3: document.documentElement.style.getPropertyValue('--bcv-ink3') }));
  const liveRows = THEME.shades(pinkA, themedNow.dark, 9);
  check(themedNow.themed && themedNow.tabIcons.join(',') === liveRows.map((r) => r.icon).join(',') && new Set(themedNow.tabIcons).size === 5 && themedNow.navColor === inkOver(themedNow.dark) && themedNow.h1 === inkOver(themedNow.dark) && themedNow.headPic && themedNow.headLayers === 3 && /^url\("data:image\/png/.test(themedNow.headMask) && themedNow.headMask.includes('linear-gradient(to left') && themedNow.headPicVal === 'url("data:image' && themedNow.headInk && themedNow.headPaper === rgbOf(THEME.mix(themedNow.dark ? '#000000' : '#f2f2f6', pinkA, THEME.CAST[themedNow.dark ? 'dark' : 'light'].bg)) && /^#[0-9a-f]{6}$/.test(themedNow.headVeil) && themedNow.sideLayers === 3 && themedNow.sideUrl === 'url("data:image/png;base64' && themedNow.sideInk && themedNow.sidePaper === rgbOf(THEME.mix(themedNow.dark ? '#000000' : '#f2f2f6', pinkA, THEME.CAST[themedNow.dark ? 'dark' : 'light'].bg)) && /^#[0-9a-f]{6}$/.test(themedNow.sideVeil) && themedNow.sideBlurFilter === 'none' && themedNow.headBlurFilter === 'none' && themedNow.note === pinkA, `the page wears the theme: five shades of pink cycling down the rail, white words over the sidebar's photo, the Dashboard header's drawn photo blurring towards its white title, the colour in the first-paint note: ${JSON.stringify(themedNow)}`);
  // the photo runs up through the widgets' bar: the bar clear over it, the layers reaching up by its height; the controls over
  // photos are frosted; and the greys round the colour take a soft cast of it — the ground, the cards, the secondary ink
  const CAST = THEME.CAST[themedNow.dark ? 'dark' : 'light'];
  const castBg = THEME.mix(themedNow.dark ? '#000000' : '#f2f2f6', pinkA, CAST.bg), castCard = THEME.mix(themedNow.dark ? '#1c1c1e' : '#ffffff', pinkA, CAST.card);
  check(themedNow.strip && themedNow.barBg === 'rgba(0, 0, 0, 0)' && themedNow.barBlur === 'none' && themedNow.picTop === '-44px' && /blur/.test(themedNow.segGlass) && themedNow.segColor === ink2Over(themedNow.dark) && /blur/.test(themedNow.footGlass) && themedNow.footColor === inkOver(themedNow.dark) && /blur/.test(themedNow.activeGlass) && themedNow.bg === castBg && themedNow.cardBg === rgbOf(castCard) && themedNow.ink3 === THEME.mix('#8e8e93', pinkA, CAST.ink3), `the header's photo runs up through the widgets' bar (clear, no blur, the layers up by 44px), the switcher, the active row and the foot are frosted, and the ground, the cards and the inks take a soft cast of pink: ${JSON.stringify({ strip: themedNow.strip, barBg: themedNow.barBg, barBlur: themedNow.barBlur, picTop: themedNow.picTop, segGlass: themedNow.segGlass, segColor: themedNow.segColor, footGlass: themedNow.footGlass, footColor: themedNow.footColor, activeGlass: themedNow.activeGlass, bg: themedNow.bg, castBg, cardBg: themedNow.cardBg, castCard, ink3: themedNow.ink3 })}`);
  ...
  await page.waitForTimeout(300);
  const headAfter = await headAt();
  check(headBefore.pos === 'sticky' && headBefore.top === headBefore.bar && headBefore.firstCard >= headBefore.bottom + 8 && headAfter.top === headAfter.bar && headAfter.firstCard < headAfter.bottom, `the header with a photo sits at the bar's foot with the cards clear under it, and stays there as the page scrolls: ${JSON.stringify({ before: headBefore, after: headAfter })}`);
  await page.mouse.wheel(0, -400);
  await page.waitForTimeout(300);
  await waitText('.bcv-stat__value', /^\d+$/);
  const picCard = await page.$eval('.bcv-stat[data-stat="today"]', (e) => ({ pic: e.classList.contains('bcv-stat--pic'), inked: e.classList.contains('has-ink'), layers: e.querySelectorAll('.bcv-stat__pic').length, val: e.style.getPropertyValue('--bcv-pic-ink').slice(0, 19), paper: getComputedStyle(e).backgroundColor, inkColor: getComputedStyle(e.querySelector('.bcv-stat__pic')).backgroundColor, sharpMask: getComputedStyle(e.querySelector('.bcv-stat__pic')).maskImage.slice(0, 19), veil: e.style.getPropertyValue('--bcv-veil'), label: getComputedStyle(e.querySelector('.bcv-label')).color, mask: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).maskImage, withPics: document.querySelectorAll('.bcv-stat--pic').length, blurFilter: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).filter, veilBg: getComputedStyle(e.querySelector('.bcv-stat__pic--veil')).backgroundImage }));
  const nearC = (a, b) => { const p = (c) => { const m = String(c).match(/^color\(srgb ([\d.]+) ([\d.]+) ([\d.]+)/); return m ? m.slice(1, 4).map((v) => Math.round(parseFloat(v) * 255)) : (String(c).match(/\d+/g) || []).slice(0, 3).map(Number); }; const x = p(a), y = p(b); return x.length === 3 && y.length === 3 && x.every((v, i) => Math.abs(v - y[i]) <= 2); }; // (a color-mix() computes to color(srgb …) in Chromium)
  check(picCard.pic && picCard.inked && picCard.layers === 3 && picCard.val === 'url("data:image/png' && picCard.paper === rgbOf(castCard) && nearC(picCard.inkColor, rgbOf(THEME.inkOn(castCard, themedNow.dark))) && picCard.sharpMask === 'url("data:image/png' && /^#[0-9a-f]{6}$/.test(picCard.veil) && picCard.label === inkOver(themedNow.dark) && /^url\("data:image\/png/.test(picCard.mask) && picCard.mask.includes('radial-gradient') && picCard.withPics === 1 && picCard.blurFilter === 'none' && picCard.veilBg === 'none', `the Due today counter carries Dusk tone on tone — the card's own colour as the paper, the mask a shade lighter, the blurred mask under the fade, no filter, no veil — with the look's words over it; the other five are plain: ${JSON.stringify({ ...picCard, mask: picCard.mask.slice(0, 40) })}`);
  await shot(page, '32h-themed-dashboard');
  // the sidebar's photo holds still as the sidebar scrolls: sticky at the top, the height of the sidebar
  const sideScroll = await page.evaluate(() => { const side = document.querySelector('.bcv-side'); const pic = side.querySelector('.bcv-side__pic'); const layer = pic.querySelector('.bcv-side__pic-sharp'); const before = Math.round(layer.getBoundingClientRect().top); side.scrollTop = 10000; const after = layer.getBoundingClientRect(); return { scrollable: side.scrollHeight > side.clientHeight, scrolled: side.scrollTop, before, top: Math.round(after.top), sideTop: Math.round(side.getBoundingClientRect().top), h: Math.round(after.height), clientH: side.clientHeight, sticky: getComputedStyle(pic).position, brandTop: Math.round(side.querySelector('.bcv-brand').getBoundingClientRect().top) }; });
  ...
  // Ready-made: a tile places a colour and three of the drawn scenes at once — one on the sidebar, one on
  ...
  // no two counters wear the same drawing: each gets its own variation of the scene (nine drawings of each)
  const distinctOf = (sel) => page.evaluate((q) => { const r = document.querySelector('#bcv-setup').shadowRoot; const v = [...r.querySelectorAll(q)].map((e) => { const l = e.querySelector('.pz__pic--sharp'); return l?.style.getPropertyValue('--ink') || l?.style.getPropertyValue('--pic') || ''; }); return { n: v.length, distinct: new Set(v).size, blank: v.filter((x) => !x).length }; }, sel);
  check(await eventually(async () => { const d = await distinctOf('.pz__card'); return d.n === 6 && d.distinct === 6 && d.blank === 0; }, 8000), `every counter wears its own drawing of the scene, no two the same: ${JSON.stringify(await distinctOf('.pz__card'))}`);
  const readyState = await page.evaluate(() => { const r = document.querySelector('#bcv-setup').shadowRoot; r.querySelector('#pzNext').click(); return new Promise((res) => setTimeout(() => { r.querySelector('#pzNext').click(); setTimeout(() => res({ h1: r.querySelector('.pz__h1').textContent, heads: r.querySelectorAll('.pz__hcard.has-pic').length, note: r.querySelector('#pzNote').textContent }), 400); }, 400)); });
  check(readyState.h1 === 'Page headers' && readyState.heads === 9 && readyState.note === '9 of 9', `and every one of the nine headers wears its scene: ${JSON.stringify(readyState)}`);
  check(await eventually(async () => { const d = await distinctOf('.pz__hcard'); return d.n === 9 && d.distinct === 9 && d.blank === 0; }, 8000), `and every header its own drawing too: ${JSON.stringify(await distinctOf('.pz__hcard'))}`);
  await shot(page, '32e7-personalize-ready');
  // (nothing is written until Save: the reload below leaves it behind)
  ...
  check(await eventually(async () => !(await page.$('html.bcv-themed')) && !(await page.$('.bcv-side__pic')) && !(await page.$('.bcv-head--pic')), 5000) && themeReset.accent === '' && themeReset.name === 'Regular' && !(await page.evaluate(() => document.documentElement.style.getPropertyValue('--bcv-bg') || document.documentElement.classList.contains('bcv-head-pic'))) && !themeReset.images?.side && !Object.keys(themeReset.images?.cards || {}).length && !Object.keys(themeReset.images?.headers || {}).length && (await page.evaluate(() => JSON.parse(localStorage.getItem('bcv:early') || '{}').accent)) === '' && ((await colourNow()) || '').toLowerCase() === colourWas.toLowerCase() && (await page.$eval('.bcv-h1', (e) => getComputedStyle(e).color)) === 'rgb(28, 28, 30)', `Save writes it all and Open Canvas reloads: the regular colours, no photos, the course's colour back, the title ink again: ${JSON.stringify({ ...themeReset, images: undefined })}`);
...
<script>
  ...
  const island = await page.$eval('#bcv-pins .bcv-island', (e) => { const r = e.getBoundingClientRect(); const seg = [...e.querySelectorAll('.bcv-island__segbtn')]; const open = e.querySelector('.bcv-island__open'); const inside = (b) => { const s = b.getBoundingClientRect(); return s.left >= r.left && s.right <= r.right && s.top >= r.top && s.bottom <= r.bottom; }; return { w: Math.round(r.width), h: Math.round(r.height), time: e.querySelector('.bcv-island__time').textContent, label: e.querySelector('.bcv-island__label').textContent, main: e.querySelector('.bcv-island__main').getAttribute('aria-label'), cancel: e.querySelector('.bcv-island__cancel').getAttribute('aria-label'), seg: seg.map((b) => `${b.textContent}${b.classList.contains('is-on') ? '*' : ''}`).join(' '), segPressed: seg.map((b) => b.getAttribute('aria-pressed')).join(' '), open: open?.title || '', openText: open?.textContent.trim() || '', fits: seg.every(inside) && !!open && inside(open), color: getComputedStyle(e.querySelector('.bcv-island__time')).color, expanded: e.getAttribute('aria-expanded'), dur: getComputedStyle(e).transitionDuration }; });
  check(island.w === 250 && island.h === 84 && /^2[45]:\d\d$/.test(island.time) && island.label === 'Focus' && island.main === 'Pause' && island.cancel === 'End' && island.seg === 'Focus* Break' && island.segPressed === 'true false' && island.open === 'Open the timer' && island.openText === 'Open' && island.fits && island.color === 'rgb(255, 149, 0)' && island.expanded === 'true' && island.dur.split(',').every((d) => Math.round(parseFloat(d) * 1000) >= 480 && Math.round(parseFloat(d) * 1000) <= 520), `the pointer over the pin swells it into the island over half a second, no press needed: End and Pause, the phase, the count large in orange, a Focus | Break switch under it and an Open button, all within the island (${JSON.stringify(island)})`);
  ...
  await page.click('#bcv-pins .bcv-island__segbtn[data-phase="break"]');
  await eventually(async () => (await raw('#bcv-pins .bcv-island__label'))[0] === 'Break', 3000);
  const afterBreak = await focusStored();
  check(afterBreak.phase === 'short' && afterBreak.endAt > Date.now() + 4 * 60 * 1000 && /^[45]:\d\d$/.test((await raw('#bcv-pins .bcv-island__time'))[0]) && (await page.$eval('#bcv-pins .bcv-island__seg', (e) => [...e.children].map((b) => `${b.classList.contains('is-on')}/${b.getAttribute('aria-pressed')}`).join())) === 'false/false,true/true' && (await page.$eval('#bcv-pins .bcv-island', (e) => getComputedStyle(e).getPropertyValue('--bcv-live-color').trim())) === '#34c759', 'Break on the island\'s switch moves to a short break at once, still going, in green, the switch now on Break');
  ...
  check((await page.$eval('#bcv-pins .bcv-island', (e) => e.classList.contains('is-paused'))) && (await raw('#bcv-pins .bcv-island__label'))[0] === 'Paused' && (await page.$eval('#bcv-pins .bcv-island__seg', (e) => !e.hidden && [...e.children].every((b) => !b.disabled))) && (await inPage('focusActive')) === false, 'Pause on the island holds the count; the switch stays');
  // the switch works while paused too: the other phase, its full time waiting, still paused
  await page.click('#bcv-pins .bcv-island__segbtn[data-phase="focus"]');
  await eventually(async () => (await page.$eval('#bcv-pins .bcv-island__segbtn[data-phase="focus"]', (e) => e.classList.contains('is-on'))), 3000);
  const pausedSwitch = { label: (await raw('#bcv-pins .bcv-island__label'))[0], time: (await raw('#bcv-pins .bcv-island__time'))[0], rec: await focusStored(), paused: await page.$eval('#bcv-pins .bcv-island', (e) => e.classList.contains('is-paused')), color: await page.$eval('#bcv-pins .bcv-island', (e) => getComputedStyle(e).getPropertyValue('--bcv-live-color').trim()) };
  check(pausedSwitch.label === 'Paused' && pausedSwitch.time === '25:00' && pausedSwitch.rec.phase === 'focus' && pausedSwitch.rec.left === 1500 && !pausedSwitch.rec.endAt && pausedSwitch.paused && pausedSwitch.color === '#ff9500', `Focus on the switch while paused moves to a focus block, its full 25:00 waiting, still paused, orange again (${JSON.stringify({ ...pausedSwitch, rec: pausedSwitch.rec && { phase: pausedSwitch.rec.phase, left: pausedSwitch.rec.left, endAt: pausedSwitch.rec.endAt } })})`);
  // and back: Break while paused is the short break again, its full 5:00 waiting, still paused, green (the session stays on the break from here, as the setter checks further down expect)
  await page.click('#bcv-pins .bcv-island__segbtn[data-phase="break"]');
  await eventually(async () => (await page.$eval('#bcv-pins .bcv-island__segbtn[data-phase="break"]', (e) => e.classList.contains('is-on'))), 3000);
  const pausedBack = { label: (await raw('#bcv-pins .bcv-island__label'))[0], time: (await raw('#bcv-pins .bcv-island__time'))[0], rec: await focusStored(), color: await page.$eval('#bcv-pins .bcv-island', (e) => getComputedStyle(e).getPropertyValue('--bcv-live-color').trim()) };
  check(pausedBack.label === 'Paused' && pausedBack.time === '5:00' && pausedBack.rec.phase === 'short' && pausedBack.rec.left === 300 && !pausedBack.rec.endAt && pausedBack.color === '#34c759', `and Break again while paused: the short break, its full 5:00 waiting, still paused, green (${pausedBack.label} · ${pausedBack.time} · ${pausedBack.rec.phase} · ${pausedBack.rec.left})`);
  ...
  await openTool('ptable');
  await page.waitForTimeout(300);
  check((await page.$$('.bcv-pt__cell')).length === 118 && (await toolSub()) === '118 elements' && (await page.$$('.bcv-pt__chip')).length === 10 && (await texts('.bcv-pt__hint'))[0] === 'Press an element, or type its symbol, name or number.' && (await page.$eval('.bcv-pt__cell[data-symbol="He"]', (e) => e.style.gridColumn === '18' && e.style.gridRow === '1')), 'the periodic table opens with all 118 elements in place, ten kinds in the legend, and a card waiting for a press');
  await page.click('.bcv-pt__cell[data-symbol="Fe"]');
  check((await texts('.bcv-pt__name'))[0] === 'Iron' && (await texts('.bcv-pt__bignum'))[0] === '26' && (await texts('.bcv-pt__cat'))[0] === 'Transition metal · 55.845 u' && (await texts('.bcv-pt__factv')).join(' | ') === 'Group 8 · Period 4 · d-block | Solid | [Ar] 3d6 4s2 | 1.83 | 1538 °C · 2861 °C | 7.874 g/cm³' && /^Iron is a chemical element/.test((await texts('.bcv-pt__summarytext'))[0]) && (await page.$eval('.bcv-pt__wiki', (e) => e.href)) === 'https://en.wikipedia.org/wiki/Iron' && (await toolSub()) === 'Iron · 26', `Iron pressed: its card in the table's corner, the facts, a line about it (${(await texts('.bcv-pt__factv')).join(' | ')})`);
  await shot(page, '46-periodic-table');
  await page.keyboard.press('ArrowRight');
  check((await texts('.bcv-pt__name'))[0] === 'Cobalt' && (await page.$eval('.bcv-pt__cell.is-sel', (e) => e.dataset.symbol)) === 'Co', 'the arrow keys walk the table');
  await page.fill('.bcv-pt__search', 'gold');
  check((await page.$$eval('.bcv-pt__cell.is-match', (els) => els.map((e) => e.dataset.symbol))).join(',') === 'Au' && (await page.$$('.bcv-pt__cell.is-dim')).length === 117, 'a search lights the matches and dims the rest');
  await page.press('.bcv-pt__search', 'Enter');
  check((await texts('.bcv-pt__name'))[0] === 'Gold' && (await page.$eval('.bcv-pt__search', (e) => e.value)) === '' && !(await page.$('.bcv-pt__cell.is-dim')) && (await texts('.bcv-pt__cat'))[0] === 'Transition metal · 196.967 u', 'Enter picks the first match and clears the search');
  ...
  console.log('grade needed');
  {
    ...
    await openTool('pdfx');
    check((await toolSub()) === 'Runs on this device. Nothing is uploaded.' && (await texts('.bcv-tool__head .bcv-seg__btn')).join('/') === 'Merge/Split' && (await texts('.bcv-conv__droptitle'))[0] === 'Drop PDFs here, or choose them' && (await texts('.bcv-conv__dropsub'))[0] === 'Several to merge into one · one to trim down', 'Merge & split opens on a drop zone, Merge and Split in its head, and says nothing is uploaded');
    ...
    const ntTray = () => texts('.bcv-pdfx__tile--tray .bcv-pdfx__tilen').then((t) => t.join(','));
    check(await eventually(() => page.$$('.bcv-pdfx__file').then((r) => r.length === 2), 10000) && (await texts('.bcv-pdfx__filehead')).join(' | ') === 'Chapter.pdf 2 of 2 used Add all | Lab.pdf 3 of 3 used Add all' && (await texts('.bcv-tool__note'))[0] === '1 file that is not a PDF ignored' && (await ntTray()) === '1,2,1,2,3' && (await texts('.bcv-pdfx__srchead--top'))[0] === 'SOURCE PAGES 2 files · drag pages down' && (await texts('.bcv-pdfx__outline'))[0] === 'merged.pdf · 5 pages' && (await texts('.bcv-pdfx__run'))[0] === 'Merge' && (await page.$eval('.bcv-pdfx__run', (e) => e.classList.contains('is-on') && !e.disabled)) && (await page.$eval('.bcv-pdfx__namein', (e) => e.placeholder)) === 'merged', `two PDFs lay their pages out, every page in the merged document to start, the text file named as ignored (${(await texts('.bcv-pdfx__filehead')).join(' | ')} · ${await ntTray()})`);
    check(await eventually(() => page.$$eval('.bcv-pdfx__tilebody.has-thumb', (e) => e.length === 10), 20000), 'every page gets its thumbnail drawn, in the source list and the tray');
    await shot(page, '42-merge-split');
    await page.click('.bcv-pdfx__tray .bcv-tool__link');
    check((await texts('.bcv-pdfx__trayempty'))[0] === 'Drag pages here in the order you want them' && (await texts('.bcv-pdfx__outline'))[0] === 'Nothing in the merged document yet' && (await page.$eval('.bcv-pdfx__run', (e) => e.disabled && !e.classList.contains('is-on'))) && (await texts('.bcv-pdfx__filehead .bcv-pdfx__hint')).join('|') === 'none used|none used' && !(await page.$('.bcv-pdfx__tick')), 'Clear empties the merged document, the ticks go and Merge waits');
    await page.click('.bcv-pdfx__file:nth-child(1) .bcv-pdfx__tile--src:nth-child(2)');
    await page.waitForTimeout(150);
    check((await ntTray()) === '2' && (await texts('.bcv-pdfx__outline'))[0] === 'Add one more page to merge' && (await page.$$eval('.bcv-pdfx__tick', (e) => e.length)) === 1 && (await page.$eval('.bcv-pdfx__file:nth-child(1) .bcv-pdfx__tile--src:nth-child(2)', (e) => e.classList.contains('is-used'))), 'a page pressed goes on the end of the merged document and gets its tick');
    const ntSrc = await (await page.$('.bcv-pdfx__file:nth-child(2) .bcv-pdfx__tile--src:nth-child(1)')).boundingBox();
    const ntSlot = await (await page.$('.bcv-pdfx__tile--tray[data-slot="0"]')).boundingBox();
    await page.mouse.move(ntSrc.x + ntSrc.width / 2, ntSrc.y + ntSrc.height / 2);
    await page.mouse.down();
    await page.mouse.move(ntSrc.x + 40, ntSrc.y + 60, { steps: 4 });
    await page.mouse.move(ntSlot.x + 4, ntSlot.y + 10, { steps: 6 });
    await page.waitForTimeout(100);
    const ntGhost = (await page.$$('.bcv-pdfx__ghost')).length === 1 && (await page.$eval('.bcv-pdfx__traybox', (e) => e.classList.contains('is-over'))) && (await page.$eval('.bcv-pdfx__file:nth-child(2) .bcv-pdfx__tile--src:nth-child(1)', (e) => e.classList.contains('is-dragged')));
    await shot(page, '42b-merge-drag');
    await page.mouse.up();
    await page.waitForTimeout(200);
    check(ntGhost && (await ntTray()) === '1,2' && (await page.$$eval('.bcv-pdfx__tile--tray', (els) => els.map((e) => e.style.getPropertyValue('--c')).join(','))) === '#ff9500,#0a84ff' && !(await page.$('.bcv-pdfx__ghost')) && (await texts('.bcv-pdfx__outline'))[0] === 'merged.pdf · 2 pages', `a page dragged lands where it is dropped, a ghost under the pointer and the tray lit on the way, each page in its file's colour (${await ntTray()})`);
    const ntS1 = await (await page.$('.bcv-pdfx__tile--tray[data-slot="1"]')).boundingBox();
    const ntS0 = await (await page.$('.bcv-pdfx__tile--tray[data-slot="0"]')).boundingBox();
    await page.mouse.move(ntS1.x + ntS1.width / 2, ntS1.y + ntS1.height / 2);
    await page.mouse.down();
    await page.mouse.move(ntS1.x + 10, ntS1.y + 30, { steps: 3 });
    await page.mouse.move(ntS0.x + 3, ntS0.y + 20, { steps: 5 });
    await page.mouse.up();
    await page.waitForTimeout(200);
    check((await ntTray()) === '2,1', 'pages in the merged document drag about to reorder');
    await page.click('.bcv-pdfx__file:nth-child(2) .bcv-tool__link--blue');
    await page.waitForTimeout(150);
    await page.click('.bcv-pdfx__tile--tray[data-slot="4"] .bcv-pdfx__x');
    await page.waitForTimeout(150);
    await page.fill('.bcv-pdfx__namein', 'Week 4 pack');
    check((await ntTray()) === '2,1,1,2' && (await texts('.bcv-pdfx__filehead .bcv-pdfx__hint')).join('|') === '1 of 2 used|2 of 3 used' && (await texts('.bcv-pdfx__outline'))[0] === 'Week 4 pack.pdf · 4 pages', `Add all puts a whole file in, the × takes a page out, and the name typed shows in what you get (${await ntTray()})`);
    const ntMerged = await ntDownload(() => page.click('.bcv-pdfx__run'));
    const ntMergedText = ntMerged.bytes.toString('latin1');
    check(ntMerged.name === 'Week 4 pack.pdf' && ntPages(ntMerged.bytes) === 4 && /Second page text/.test(ntMergedText) && /Page one/.test(ntMergedText) && /Page two/.test(ntMergedText) && !/Chapter One/.test(ntMergedText) && !/Page three/.test(ntMergedText) && (await texts('.bcv-pdfx__run'))[0] === 'Merged' && (await page.$eval('.bcv-pdfx__run', (e) => e.classList.contains('is-done'))), `Merge downloads one PDF of just the pages in the tray, in that order, and the button says Merged (${ntMerged.name}, ${ntPages(ntMerged.bytes)} pages)`);
    await page.click('.bcv-tool__head .bcv-seg__btn[data-value="split"]');
    await page.waitForTimeout(400);
    check((await texts('.bcv-pdfx__filerow .bcv-pdfx__filemeta'))[0] === '2 pages · 1 KB' && (await raw('.bcv-pdfx__which .bcv-picker__label'))[0] === 'Chapter.pdf' && (await texts('.bcv-pdfx__filerow .bcv-pdfx__replace'))[0] === 'Replace' && (await texts('.bcv-pdfx__splittop .bcv-pdfx__srchead'))[0] === 'PAGES all 2 kept' && (await page.$$('.bcv-pdfx__tile--split')).length === 2 && (await texts('.bcv-pdfx__outline'))[0] === 'Remove a page to create a new PDF' && (await texts('.bcv-pdfx__run'))[0] === 'Save PDF' && (await page.$eval('.bcv-pdfx__run', (e) => e.disabled)) && (await page.$eval('.bcv-pdfx__namein', (e) => e.placeholder)) === 'Chapter-trimmed', 'Split shows the file, which one to cut, its pages all kept, and waits for a page to be taken out');
    await page.click('.bcv-pdfx__tile--split:nth-child(1) .bcv-pdfx__x');
    await page.waitForTimeout(150);
    check((await texts('.bcv-pdfx__splittop .bcv-pdfx__srchead'))[0] === 'PAGES 1 removed · 1 kept Restore all' && (await page.$$('.bcv-pdfx__tile--split.is-gone .bcv-pdfx__strike')).length === 1 && (await page.$$('.bcv-pdfx__x.is-restore')).length === 1 && (await texts('.bcv-pdfx__outline'))[0] === 'Chapter-trimmed.pdf · 1 page · 1 removed' && (await page.$eval('.bcv-pdfx__run', (e) => e.classList.contains('is-on') && !e.disabled)), 'a page taken out is struck through and counted, and the trimmed PDF is offered');
    await shot(page, '42c-split');
    const ntTrim = await ntDownload(() => page.click('.bcv-pdfx__run'));
    check(ntTrim.name === 'Chapter-trimmed.pdf' && ntPages(ntTrim.bytes) === 1 && /Second page text/.test(ntTrim.bytes.toString('latin1')) && !/Chapter One/.test(ntTrim.bytes.toString('latin1')) && (await texts('.bcv-pdfx__run'))[0] === 'Saved', `Save PDF downloads the pages kept, and the button says Saved (${ntTrim.name}, ${ntPages(ntTrim.bytes)} pages)`);
    await page.click('.bcv-pdfx__splittop .bcv-tool__link--blue');
    await page.waitForTimeout(150);
    check((await texts('.bcv-pdfx__splittop .bcv-pdfx__srchead'))[0] === 'PAGES all 2 kept' && !(await page.$('.bcv-pdfx__tile--split.is-gone')) && (await texts('.bcv-pdfx__outline'))[0] === 'Remove a page to create a new PDF' && await eventually(() => page.$eval('.bcv-pdfx__run', (e) => e.disabled && e.textContent === 'Save PDF'), 4000), 'Restore all puts every page back, and once Saved has faded the button waits again');
    await closeTool();
  ...
  await sw.evaluate(() => self.BCV.api.storage.local.set({ 'tools:pins': ['cite', 'calc', 'graph', 'ptable', 'conv', 'fc'], 'tools:cite:style': 'mla', 'tools:citations': [
  ...
  // every capsule and panel swells over the same half second as the timer's island, on the same spring; the big shadow waits for the box to stand still (a shadow redrawn on every frame of a resize is what dropped frames)
  const readCapsuleTiming = () => page.$eval('#bcv-pins .bcv-pin[data-tool="calc"]', (e) => { const cs = getComputedStyle(e); const rootCs = getComputedStyle(document.documentElement); const before = getComputedStyle(e, '::before'); return { dur: cs.transitionDuration, ease: cs.transitionTimingFunction.slice(0, 7), island: rootCs.getPropertyValue('--bcv-t-island').trim(), islandEase: rootCs.getPropertyValue('--bcv-spring-island').trim().slice(0, 7), faceShadow: getComputedStyle(e.querySelector('.bcv-quick__face')).boxShadow, shadow: before.boxShadow.includes('36px'), shadowShown: before.visibility === 'visible' && before.opacity === '1' }; });
  let capsuleTiming = await readCapsuleTiming();
  for (let i = 0; i < 30 && !capsuleTiming.shadowShown; i++) { await page.waitForTimeout(100); capsuleTiming = await readCapsuleTiming(); } // (the shadow fades in once the box stands still)
  check(capsuleTiming.dur.split(',').every((d) => Math.abs(parseFloat(d) * 1000 - parseFloat(capsuleTiming.island)) < 1) && capsuleTiming.ease === capsuleTiming.islandEase && parseFloat(capsuleTiming.island) >= 480 && parseFloat(capsuleTiming.island) <= 520 && capsuleTiming.faceShadow === 'none' && capsuleTiming.shadow && capsuleTiming.shadowShown, `a capsule swells over the same half second as the timer's island, on the same spring; its shadow is a layer of its own, shown once it stands still (${JSON.stringify(capsuleTiming)})`);
  ...
  await page.waitForTimeout(120);
  check((await texts(qpSel('.bcv-qpt__line')))[0] === 'Iron · 26 · 55.845', 'the element under the pointer is named, with its number and mass');
  await page.fill(qpSel('.bcv-quick__input'), 'Na');
  await page.waitForTimeout(120);
  ...
  await page.waitForFunction(() => !document.querySelector('.bcv-toast'), null, { timeout: 5000 });
  await page.click('.bcv-stats .bcv-stat:nth-child(1)');
  await page.waitForSelector('.bcv-sheet-ov > .bcv-sheet', { timeout: 5000 });
  await page.waitForTimeout(500); // (in and settled)
  const stuckAt = Date.now();
  await runInApp('stuck');
  await page.waitForTimeout(300);
