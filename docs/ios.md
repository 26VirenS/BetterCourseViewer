...
- **The first launch opens the guided setup** (`/?bcv=setup` on the Canvas site, after Canvas's own sign-in), over Today: a word-mark, then the courses to favourite (with nicknames), the GPA goal, tracking and a target per course, a read-back of those answers, and *Open Canvas* goes straight into a tour of the phone layout (a phone has no sidebar and no dashboard views to choose between, so those two desktop steps are left out). It can be run again from the account sheet under the avatar on Today. The settings page's own "Guided setup" section is hidden in the app, like "Canvas sites".
