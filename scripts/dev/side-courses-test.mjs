...
try {
  ...
  const setMode = (mode) => sw.evaluate(async (m) => {
    ...
    await self.BCV.api.storage.local.set({ 'setup:offered': true, 'setup:done': true, 'setup:flow': 2 }); // the flow marker too: the background's migration clears the flags for an older flow, and may run after this
  }, mode);
