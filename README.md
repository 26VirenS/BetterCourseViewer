...
- Item views for assignments (rubric, submission, comments), discussion threads (nested replies, reply box), pages, quizzes and the syllabus. A **rubric** is drawn from its parts rather than flattened into a line: a criterion's long description is Canvas rich text, so its own bullets, line breaks and links are drawn as written (they used to show as markup); every rating it offers is listed as a chip, the one the work was given filled in with its score and the marker's note beside it.
...
**Nothing wedges** – a press on a row that is already loading is ignored so a double press does not start the same screen twice, but that runs out: a load left lit for good — a tab put away mid-load, a reply that never came, a timer a hidden tab never ran — no longer makes its row dead for the rest of the page's life, and coming back to the tab puts out a wash that was left sweeping. Coming back is also not counted as a sign of life any more: the return always arrives before the press, so counting it defeated the stale-page reload entirely — after five minutes away the first press reloads, as it was meant to.
