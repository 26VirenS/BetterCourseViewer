...
(function () {
  ...
  let STEPS = ALL;
  // (a phone has no sidebar and no dashboard views to choose between; the look, the colour and the
  // photos follow in Personalize — content/app/personalize.js — which the read-back leads into)
  const PHONE_STEPS = ['courses', 'grades'];
  ...
  async function open(app) {
    ...
    if (only === 'personalize') personalize({ standalone: true }); // the look, the colour and the photos on their own, in the card's place
    else go(0, 1);
    playIntro();
  }
  ...
  function footer({ next = st.step === STEPS.length - 1 ? 'Finish' : 'Continue', onNext, disabled = false, back = st.step > 0 && !isDone() } = {}) {
  ...
  function done() {
    ...
    footer({ next: 'Continue to appearance', back: true, onNext: () => finish() });
  }
  ...
  /** Personalize (content/app/personalize.js) in this overlay, in the setup card's place: after the
   *  read-back, or on its own. Its Open Canvas loads the page afresh — with the welcome on it when
