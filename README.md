...
**Guided setup** – opens on the settings page after install (Safari and Chrome ask for your Canvas site first) and on the app's first launch (iOS), then continues on your own Canvas page: the courses to favourite (the same list every screen follows), your grades (a term GPA goal, tracking over time, a target grade per course), the smart panel with the steps to get a Claude or ChatGPT key (Gemini is coming soon), and a tour of the interface on the real pages, one spotlight at a time. Run it again any time from Settings → Guided setup, the toolbar popup, or the account sheet on a phone.
