...
**Never a broken card** – a screen that throws, that would land nothing but an error, or that has not drawn after fifteen seconds gives way to Canvas's own page for that URL, with a note saying so, rather than a card that says something failed. **Cache first** – an entry past its time but under forty-five minutes old is drawn at once and refreshed behind the page; if the fresh answer differs, the screen redraws in place with the scroll kept, unless you are mid-way through something (a sheet, a quiz, typing).
