...
(function () {
  ...
  const FIELDS = [
    ...
    { key: 'settle', max: 5, label: 'Let it finish signing in first', words: ['Take it at once', 'Quiet for 1s', 'Quiet for 2s', 'Quiet for 3s', 'Quiet for 5s', 'Quiet for 10s'] },
  ];
  ...
  const SHIPPED = { mode: 2, windows: 1, wait: 2, blank: 0, close: 1, log: 0, toast: 0, settle: 2 };
  // what to turn on to find out what a browser actually reports: catch everything, close nothing
  const EVERYTHING = { mode: 3, windows: 1, wait: 5, blank: 1, close: 0, log: 1, toast: 1, settle: 2 };

  const clamp = (n, max) => Math.max(0, Math.min(max, Number(n) || 0));
  /** The settings as a code: SC2-31510112. */
  function encode(s) {
    return `SC2-${FIELDS.map((f) => clamp(s?.[f.key], f.max)).join('')}`;
  }
  ...
  function decode(code) {
    ...
    if (digits.length !== FIELDS.length) return { ok: false, settings: { ...SHIPPED }, message: `A code is SC2- and ${FIELDS.length} digits.` };
    const settings = {};
  ...
  BCV.devcode = { FIELDS, WAIT_MS, SETTLE_MS, SHIPPED, EVERYTHING, encode, decode, explain, KEY: 'dev:capture' };
})();
