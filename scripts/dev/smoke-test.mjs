...
try {
  ...
  const dusk = await page.$eval(pz('.pz__card[data-target="today"]'), (e) => ({ pic: e.classList.contains('has-pic'), layers: e.querySelectorAll('.pz__pic').length, paper: getComputedStyle(e.querySelector('.pz__pic--paper')).backgroundColor, inkColor: getComputedStyle(e.querySelector('.pz__pic--sharp')).backgroundColor, ink: e.querySelector('.pz__pic--sharp').style.getPropertyValue('--ink').slice(0, 19), veil: e.style.getPropertyValue('--veil-card'), num: getComputedStyle(e.querySelector('.pz__cn')).color, mask: getComputedStyle(e.querySelector('.pz__pic--blur')).maskImage || '', inkA: e.closest('.pz').style.getPropertyValue('--ink-a'), inkB: e.closest('.pz').style.getPropertyValue('--ink-b') }));
  const customNow = await pzVar('--A');
  check(dusk.pic && dusk.layers === 4 && dusk.paper === rgbOf(THEME.inkPair(customNow, pzDark).b) && dusk.inkColor === rgbOf(THEME.inkPair(customNow, pzDark).a) && dusk.ink === 'url("data:image/png' && /^#[0-9a-f]{6}$/.test(dusk.veil) && dusk.num === inkOver(pzDark) && /^url\("data:image\/png/.test(dusk.mask) && dusk.mask.includes('radial-gradient(150% 150% at 100%') && dusk.inkA === THEME.inkPair(customNow, pzDark).a && dusk.inkB === THEME.inkPair(customNow, pzDark).b && (await page.$eval(pz('#pzNote'), (e) => e.textContent)) === 'Custom · 1 photo', `Dusk goes on the counter in two inks — the paper in the colour's complement, the ink's mask in the colour, the blurred mask under the fade, the veil — with the look's words over it: ${JSON.stringify({ ...dusk, mask: dusk.mask.slice(0, 40) })}`);
  ...
  await page.waitForTimeout(300);
  await waitText('.bcv-stat__value', /^\d+$/);
  const picCard = await page.$eval('.bcv-stat[data-stat="today"]', (e) => ({ pic: e.classList.contains('bcv-stat--pic'), inked: e.classList.contains('has-ink'), layers: e.querySelectorAll('.bcv-stat__pic').length, val: e.style.getPropertyValue('--bcv-pic-ink').slice(0, 19), paper: getComputedStyle(e).backgroundColor, inkColor: getComputedStyle(e.querySelector('.bcv-stat__pic')).backgroundColor, sharpMask: getComputedStyle(e.querySelector('.bcv-stat__pic')).maskImage.slice(0, 19), veil: e.style.getPropertyValue('--bcv-veil'), label: getComputedStyle(e.querySelector('.bcv-label')).color, mask: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).maskImage, withPics: document.querySelectorAll('.bcv-stat--pic').length, blurFilter: getComputedStyle(e.querySelector('.bcv-stat__pic--blur')).filter, inkA: document.documentElement.style.getPropertyValue('--bcv-ink-a'), inkB: document.documentElement.style.getPropertyValue('--bcv-ink-b'), wash: getComputedStyle(e.querySelector('.bcv-stat__pic--veil')).backgroundColor }));
  const pairNow = THEME.inkPair(pinkA, themedNow.dark);
  check(picCard.pic && picCard.inked && picCard.layers === 3 && picCard.val === 'url("data:image/png' && picCard.paper === rgbOf(pairNow.b) && picCard.inkColor === rgbOf(pairNow.a) && picCard.sharpMask === 'url("data:image/png' && /^#[0-9a-f]{6}$/.test(picCard.veil) && picCard.label === inkOver(themedNow.dark) && /^url\("data:image\/png/.test(picCard.mask) && picCard.mask.includes('radial-gradient') && picCard.withPics === 1 && picCard.blurFilter === 'none' && picCard.inkA === pairNow.a && picCard.inkB === pairNow.b && /^rgba\((255, 255, 255|0, 0, 0), /.test(picCard.wash), `the Due today counter carries Dusk in two inks — pink on its complement's paper, the mask in the colour, the blurred mask under the fade, no filter — with the look's words over it; the other five are plain: ${JSON.stringify({ ...picCard, mask: picCard.mask.slice(0, 40) })}`);
