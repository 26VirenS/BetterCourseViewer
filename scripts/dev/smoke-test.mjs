...
try {
  ...
  const stepNo = () => page.$eval('.bcv-su__stepno', (e) => e.textContent.trim()).catch(() => '');
  ...
  check((await page.title()).startsWith('Guided setup'), `the tab is titled: ${await page.title()}`);
  await toStep(2);
  check((await stepNo()) === 'Step 2 of 6 · Your site' && /^On for localhost/.test((await texts('.bcv-su__h1'))[0]) && (await page.$$('.bcv-su__dot.is-done')).length === 1, `step 2 confirms the site: ${(await texts('.bcv-su__h1'))[0]}`);
  await toStep(3);
  await page.waitForSelector('.bcv-su__course .bcv-switch', { timeout: 15000 });
  const suRows = (await page.$$('.bcv-su__course')).length;
  const suOn = (await page.$$('.bcv-su__course .bcv-switch.is-on')).length;
  check(suRows >= 9 && suOn === 5 && (await texts('.bcv-su__count'))[0] === '5 courses chosen' && !(await visible('.bcv-su__courses:nth-of-type(2)')), `step 3 scanned every course (${suRows}) with the 5 favourites on; other courses fold away`);
  const offRow = await page.$('.bcv-su__courses:not([hidden]) .bcv-su__course:has(.bcv-switch.is-on)');
  const onRow = await page.$('.bcv-su__courses:not([hidden]) .bcv-su__course:has(.bcv-switch:not(.is-on))');
  const offName = await offRow.$eval('.bcv-su__course-n', (e) => e.textContent.trim());
  const onName = await onRow.$eval('.bcv-su__course-n', (e) => e.textContent.trim());
  await (await offRow.$('.bcv-switch')).click();
  await (await onRow.$('.bcv-switch')).click();
  check((await texts('.bcv-su__count'))[0] === '5 courses chosen', 'the switches keep the count');
  await toStep(4);
  const favNow = (await apiGet('/api/v1/courses?per_page=100')).filter((c) => c.is_favorite).map((c) => c.name);
  check(!favNow.includes(offName) && favNow.includes(onName) && favNow.length === 5, `Continue writes the favourites to Canvas: −${offName} +${onName}`);
  await page.goto(`${BASE}/`);
  await page.waitForSelector('.bcv-fav__dot', { timeout: 10000 });
  check((await texts('.bcv-fav')).some((t) => t.includes(onName.replace(/^F26-/, '').slice(0, 8))) || (await texts('.bcv-fav')).length === 5, 'the sidebar follows the new favourites');
  await page.goto(`${BASE}/?bcv=setup&step=3`);
  await page.waitForSelector('.bcv-su__goal-v', { timeout: 15000 });
  check((await stepNo()) === 'Step 4 of 6 · Your grades' && (await texts('.bcv-su__goal-v'))[0] === '3.70' && (await page.$$('.bcv-su__select')).length === 5, 'step 4: the goal at 3.70 and a target row per favourite course');
  await page.click('.bcv-su__step:last-child');
  check((await texts('.bcv-su__goal-v'))[0] === '3.75', 'the goal stepper');
  check((await page.$eval('#bcv-su-prior', (el) => el.getBoundingClientRect().height)) === 0, 'tracking fields stay folded until tracking is on');
  await page.click('.bcv-su__switchrow .bcv-switch');
  await page.fill('#bcv-su-prior', '3.5');
  await page.fill('#bcv-su-prior-n', '10');
  await page.selectOption('.bcv-su__select', '0');
  await toStep(5);
  const prefs = await prefsOf();
  check(prefs.gpaGoal === 3.75 && prefs.gpaTracking?.priorGpa === 3.5 && prefs.gpaTracking?.priorCourses === 10 && Object.values(prefs.gradeTargets || {}).includes(0), `grades saved where the Grades page reads them: ${JSON.stringify({ goal: prefs.gpaGoal, tracking: prefs.gpaTracking, targets: prefs.gradeTargets })}`);
  const provs = await texts('.bcv-su__prov');
  check(provs.length === 3 && /Gemini\s*Coming soon/.test(provs[2]) && (await page.$('.bcv-su__prov.is-soon[aria-disabled="true"]')) !== null, `step 5 offers Claude, ChatGPT and Gemini (coming soon): ${provs.join(' | ')}`);
  check(/console\.anthropic\.com/.test((await texts('.bcv-su__ol li'))[0]) && (await page.$$('.bcv-su__ol li')).length === 4, 'with the steps to get a Claude key');
  await page.click('.bcv-su__prov[data-provider="openai"]');
  check(/platform\.openai\.com/.test((await texts('.bcv-su__ol li'))[0]), 'ChatGPT shows its own steps');
  await page.fill('.bcv-su__key', 'sk-test-setup');
  await toStep(6);
  const savedSmart = (await sw.evaluate(() => self.BCV.settings.get())).smart;
  check(savedSmart.openaiKey === 'sk-test-setup' && savedSmart.preferred === 'openai', 'Save and continue stores the key and the preferred provider');
  await setSettings({ smart: { openaiKey: '', preferred: 'claude' } });
  check((await stepNo()) === 'Step 6 of 6 · Tour' && (await texts('.bcv-su__btn.is-primary'))[0] === 'Start the tour', 'step 6 offers the tour');
  await page.click('.bcv-su__btn.is-primary');
  await page.waitForSelector('.bcv-tour__card', { timeout: 15000 });
  check(page.url() === `${BASE}/` && (await prefsOf()).setupDone === true, 'starting the tour finishes the setup and lands on the dashboard');
