...
**Sidebar** – your school's own mark from Canvas's theme, whole inside a small rounded tile with the site name beside it (its app icon, favicon or header logo; Canvas's default assets never count, and Settings can point at another image), then Dashboard / Courses / Groups / To Do (with a count) / Calendar / Inbox (with unread count) / Grades, your favourite courses with their Canvas colours, a Dark/Light appearance switch and your account.
...
- **Mac (Safari).** Deleting the app on its own leaves the extension's storage behind inside Safari, so the app carries an uninstaller. Press **Reset everything** in Settings → Data & about (keys, history, sites and the note kept on open Canvas tabs go), then open the Simpl Courses app and choose **Simpl Courses → Uninstall Simpl Courses…**: it copies one line to the clipboard, which you paste into the Terminal (⌘Space, "Terminal", then ⌘V and Return). The app runs in the App Sandbox and can neither reach Safari's storage nor start the Terminal itself, so the Terminal does the work: it quits Safari, then shows each item as it removes it — the storage and caches Safari keeps for the extension, the app's own containers, preferences, caches and saved state — and moves the app to the Trash. If the app is already gone, download `uninstall-simpl-courses-mac.command` from the [release page](https://github.com/26VirenS/BetterCourseViewer/releases/latest) and run `bash ~/Downloads/uninstall-simpl-courses-mac.command`; a downloaded script is quarantined, so double-clicking it is refused as "damaged" while the Terminal runs it. It looks in Applications, the Desktop, Downloads and through Spotlight, under whatever name a copy was given (a second one is "Simpl Courses 2.app"), and it says what it could not remove instead of finishing quietly. `--dry-run` lists without removing, `--keep-app` removes the data only, and `--app "/path/to/Simpl Courses.app"` points it at a copy kept somewhere unusual. The source is `scripts/uninstall-mac.command`.
...
scripts/
  ...
  dev/uninstall-test.sh        the Mac uninstaller against a home folder built for it (copies, lookalikes, what it cannot remove)
