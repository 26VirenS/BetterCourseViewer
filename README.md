...
**Start a discussion** – **New discussion** on a course's or group's Discussions tab opens the same form Canvas's own editor asks for: the topic title, the topic content, the options that change how the thread behaves (threaded replies, everyone must post before seeing replies, replies can be liked) and when it opens and closes (available from and until, each a day and a time, either left alone to open now and stay open). The post is written in an editor that shows it as it will read — bold, italic, underline, headings, bulleted and numbered lists, quotes, code, links and pictures, all in place — with a source view for anyone who would rather write the HTML, which is what Canvas stores either way. A picture is uploaded to your Canvas files and dropped into the post where the caret is; a file can be attached to the topic itself, the way Canvas's own form sends one (Canvas keeps one per topic). It posts, then opens the thread it made. A course that does not let students start one says so rather than failing quietly.
