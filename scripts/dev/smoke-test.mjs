...
try {
  ...
  check(readyOn.on === 'Dusk' && readyOn.colour === 'Pink' && readyOn.A === readyPink && readyOn.side === 'url("data:image' && readyOn.cards.length === 6 && readyOn.cards.every((c) => c === 'url("data:image') && readyOn.note === 'Pink · 7 photos' && readyOn.pvMain === THEME.mix('#fbfbfd', readyPink, 0.05) && readyOn.pvCard === rgbOf(THEME.mix('#ffffff', readyPink, 0.025)), `Dusk pressed: Pink, the sidebar and all six counters with a scene each, the tile on, and the preview's grounds cast softly in pink: ${JSON.stringify(readyOn)}`);
